package kr.co.hsd.web.controller.join;
...
@Controller
@RequestMapping("/join")
public class JoinController extends BaseFormController {

    private final String JSP_PATH = "join/";
     ...
     * @return
     */
    @RequestMapping("/join_step2")
    public String join_step2(Member member, HttpServletRequest request, Model model){
        if(SecurityUtils.isFullyAuthenticated()) {
            return "redirect:/";
        }

        if(!"Y".equals(member.getAgree_use()) || !"Y".equals(member.getAgree_personal())){
            saveError(request, "잘못된 접근입니다.");
            return "redirect:/join/join_step1";
        }

        model.addAttribute("agree_use", member.getAgree_use());
        model.addAttribute("agree_personal", member.getAgree_personal());
        model.addAttribute("agree_3rd", member.getAgree_3rd());
        return JSP_PATH + "join_step2";
    }
    ...
    public String auth_cert(HttpServletResponse response, Model model) {
        ...
        String srvNo    = sirenCertProperties.getSrvNoForJoin();           // 본인실명확인 서비스번호
        ...
        String retUrl   = "32" + siteProperties.getDomain() + "/join/auth_result";   // 결과전송 url
     ...
     * @return
     */
    @RequestMapping("/join_step3")
    public String join_step3(Member member, HttpServletRequest request, Model model){
        if(SecurityUtils.isFullyAuthenticated()) {
            return "redirect:/";
        }

        if(member.getName() == null || member.getMobile_phone() == null || "".equals(member.getName()) || "".equals(member.getMobile_phone())){
            saveError(request, "잘못된 접근입니다.");
            return "redirect:/join/join_step1";
        }

        // 가입경로 추가
        member.setJoin_path("W");

        // 가입여부 확인
        List<Member> list = memberService.getMemberForFindInfo(member);
        int result = list.size();
        if(result > 0){
            saveError(request, "고객님께서는 이미 한솥도시락 회원으로 가입되어 있습니다. 로그인 화면으로 이동합니다.");
            return "redirect:/login";
        }

        model.addAttribute("name", member.getName());
        model.addAttribute("mobilePhone", member.getMobile_phone());
        model.addAttribute("di", member.getDi());
        model.addAttribute("mobileCertificationReqNum", member.getMobile_certification_req_num());
        model.addAttribute("agree_use", member.getAgree_use());
        model.addAttribute("agree_personal", member.getAgree_personal());
        model.addAttribute("agree_3rd", member.getAgree_3rd());


        return JSP_PATH + "join_step3";
    }
     ...
     * @return
     */
    @RequestMapping("/join_step4")
    public String join_step4(@RequestParam("id") String id, @RequestParam("name") String name, Model model){
        if(SecurityUtils.isFullyAuthenticated()) {
            return "redirect:/";
        }
    ...
    public String faq_list(CustomerVoice customerVoice, ModelMap model) {

        customerVoice.setOrderKey("IDX");
        customerVoice.setOrderValue("DESC");
        ...
        // 리스트 갯수
        int cnt = customerService.listCustomerVoiceCnt(customerVoice);

        // 페이징
        Pagination pagination = new Pagination(customerVoice.getPageNo(), cnt);
        customerVoice.setFromNo(pagination.getFromNo());

        // 리스트
        List<CustomerVoice> list = customerService.listCustomerVoice(customerVoice);
     ...
     * @return
     */
    @RequestMapping("/question_view/{idx}")
    public String questionView(@PathVariable("idx") Long idx, CustomerVoice customerVoice, ModelMap model){


        // 상세내용
        CustomerVoice result = customerService.getCustomerVoice(idx);
        if(result == null){
            return "redirect:/join/question_list";
        }

        Map<String, Long> customerMap = new HashMap<>();
        customerMap.put("idx", idx);
        if(getCurrentUser()!=null){
            customerMap.put("memberIdx", getCurrentUser().getIdx());
        }
        // 이전글
        CustomerVoice prev = customerService.getPrevCustomerVoice(customerMap);
        // 다음글
        CustomerVoice next = customerService.getNextCustomerVoice(customerMap);

        model.addAttribute("result", result);
        model.addAttribute("prev", prev);
        model.addAttribute("next", next);

        return JSP_PATH + "question_view";
    }
     ...
     * @return
     */
    @RequestMapping("/question_delete/{idx}")
    public String questionDelete(@PathVariable("idx") Long idx){

        // 상세내용
        customerService.deleteCustomerVoice(idx);
