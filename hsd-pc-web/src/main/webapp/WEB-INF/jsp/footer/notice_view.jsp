...
<div class="content area_footer">
    <section class="notice">
        <form:form id="listForm" name="listForm" method="get" action="${cp}/footer/notice_list">
            <input type="hidden" name="pageNo" value="${notice.pageNo}" />
            <c:forEach items="${notice.searchInfo}" var="item">
        ...
        <div class="no_view_wrap">
            <div class="area_no_tit mo_version">
                <h2 class="h2_tit h2_01"><a href="#none" onclick="goList();return false;">공지사항</a></h2>
            </div>
            <div class="notice_view mo_version cont_st02">
                <div class="area_article">
                    ...
                    <div class="subject">
                        <span class="date"><fmt:formatDate value="${result.regDttm}" pattern="yyyy-MM-dd" /></span>
                        <span class="hits"><fmt:formatNumber value="${result.hits}" type="number" /></span>
                    </div>
                </div>
                 <div class="area_cont">
...
<script type="text/javascript">
    // 목록이동
    function goList(){
        location.href = "${cp}/footer/notice_list";
    }

    // 이전글, 다음글
    function goView(idx){
        document.listForm.action = "${cp}/footer/notice_view/"+idx;
